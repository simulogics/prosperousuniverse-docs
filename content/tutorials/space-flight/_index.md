...
### FTL vs. STL travel
There are two modes of travel: FTL and FTL. Note that these two need different kinds of fuel, as you can see when you click the fuel bars of one of your ships:

![Fuel types](fuel-types.png)

Each of the dots on the Universe Map (command: MU NAV) represents a star and its planetary system. Going from one star to another means travelling a vast distance, which is only possible by FTL, i.e. Faster Than Light travel. The lines between systems represent the flight routes for FTL travel.
...
To send your ship on its way, first click FLY in its row in the Fleet buffer. In the SFC that opened up, you need to enter a planet or station ID of the ship’s destination. For example, enter “Montem”, and the Ship Flight Control will find it. Once that happened, a preview of your flight route will appear in the respective System map you just opened. In the SFX buffer, a list of travel segments appears:
...
Many planets are currently unnamed, and you’re going to need to enter their ID, which consists of the system ID followed by a letter representing the desired planet. Note that named planets also have an ID. For example, the system Moria’s ID is XH-619. Opening Montem’s Planet buffer, we can see its ID is XH-619 - for Moria - followed by “c” for Montem itself.

#### Setting fuel usage

First off, if your target is a planet, toggle “Surface landing” on or off depending on what you would like to do. In order to deliver or pick up wares on a commodity exchange on a planet, your ships needs to land. If it does not land, it will be displayed in orbit like this upon arrival:
